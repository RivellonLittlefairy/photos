...
const ACCEPTED_MIME_TYPES = new Set<DetectedImageType["mimeType"]>([
  "image/jpeg",
  "image/png",
  "image/heic",
  "image/heif",
...
async function readFileHeader(path: string, size = 64): Promise<Buffer> {
...
function detectImageType(header: Buffer): DetectedImageType | null {
  ...
  storage: multer.diskStorage({
    destination: async (_req, _file, callback) => {
      try {
        ...
        callback(error as Error, TMP_UPLOADS_DIR);
      }
    },
...
export const uploadController = {
  parseFiles(req: Request, res: Response, next: NextFunction): void {
  ...
  async upload(req: Request, res: Response): Promise<void> {
    ...
    try {
      const data = await Promise.all(
        validatedFiles.map(async ({ file, detected }) => {
